...
El flujo `.github/workflows/windows-installer.yml` compila el instalador en `windows-2022` al ejecutarlo manualmente desde **Actions → Construir instalador Windows → Run workflow**. Introduzca el enlace oficial al ZIP de PostgreSQL, su SHA-256 y el enlace oficial a WinSW con su SHA-256. `installer/prepare-payload.ps1` verifica los archivos, obtiene Node del sitio oficial y comprueba su hash. Si termina correctamente, descargue el artefacto `GestionIglesiaPro-Setup-Windows-x64` en la página de la ejecución; allí estará `GestionIglesiaPro-Setup-0.1.0.exe`. El flujo no publica una versión ni instala el programa en su PC. Verifique el resultado de la ejecución y pruebe instalación, alta del administrador y restauración en una PC Windows de ensayo antes de usar datos reales.
