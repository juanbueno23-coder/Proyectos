...
Aplicación local para Windows. La versión 0.1.0 se compiló y probó mediante instalación real en Windows Server 2022. La revisión 0.1.1 incorpora la corrección de miembros, familias y oportunidades; consulte la [versión más reciente](https://github.com/juanbueno23-coder/Proyectos/releases) y [su validación](docs/VERIFICACION.md).
...
El flujo `.github/workflows/windows-installer.yml` compila el instalador en `windows-2022` al actualizar `main` y también se puede iniciar manualmente desde **Actions → Construir instalador Windows → Run workflow**. Usa archivos alojados en los sitios oficiales de EDB y WinSW, calcula sus hashes en el ejecutor y comprueba que el segundo download coincida; esta comprobación detecta cambios entre descargas, pero **no constituye una verificación independiente del origen**. Al iniciarlo manualmente puede indicar los SHA-256 obtenidos por otro canal para comprobar el origen además de la integridad. Node se comprueba contra el manifiesto oficial. Si termina correctamente, descargue el artefacto `GestionIglesiaPro-Setup-Windows-x64` en la página de la ejecución; allí estará `GestionIglesiaPro-Setup-0.1.1.exe`. El flujo no publica una versión ni instala el programa en su PC. Verifique el resultado de la ejecución y pruebe instalación, alta del administrador y restauración en una PC Windows de ensayo antes de usar datos reales.

## Ficha de miembros y avisos

En **Miembros** seleccione ID familia existente o «Crear familia nueva automáticamente». El registro y el nuevo ID `F-0001` se guardan en la misma transacción. Nombre y apellido son los únicos campos obligatorios; cédula (11 dígitos) y correo se validan si se rellenan. La ficha muestra el ID de miembro tras guardar. En **Programación**, cada participación nueva exige elegir un miembro activo: el responsable se obtiene del directorio. «Preparar WhatsApp» abre el mensaje individual dirigido al teléfono registrado cuando se pulsa; revise y pulse Enviar dentro de WhatsApp. Abrir el enlace no acredita envío, aceptación ni entrega. La publicación al grupo existente sigue siendo manual; lea [restricciones](docs/WHATSAPP.md). No se importa el prototipo Excel.
